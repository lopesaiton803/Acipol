...
;(function(Drupal, $, Cookies) {
    ...
    Drupal.behaviors.cookiesAlgoliaInsights = {
        ...
        consentDenied: function(context) {
            if (typeof Cookies.get('_ALGOLIA') !== undefined)
                Cookies.remove('_ALGOLIA');
        },
            ...
            return this.each(function() {
                ...
                if ($element.data('status') !== 'fallback') {
                    ...
                    $button.on('click', function(event) {
                        ...
                        document.dispatchEvent(new CustomEvent('cookiesjsrSetService',{
                            detail: {
                                services
                            }
                        }));
                ...
                if (!$elementWrapper.is('.disabled')) {
                    var wrapperClasses = classes.map(function(c) {
                        return c + '--wrap';
                    });
        ...
        cookiesLegacyOverlay: function(serviceName) {
            return this.each(function() {
                ...
                if ($element.data('status') !== 'fallback') {
                    ...
                    var $acceptLink = $('<a href="#cookiesjsrAccept"/>').addClass(classStr + '--link');
                    $acceptLink.text(Drupal.t('Only accept @service cookies', {
                        '@service': ucfirst(serviceName)
                    }));
                    $acceptLink.on('click', function(event) {
                        event.preventDefault();
...
;(function(Drupal, $) {
    'use strict';
    if (Drupal.behaviors.cookiesVideo) {
        Drupal.behaviors.cookiesVideo.consentDenied = function(context) {
            $('iframe.cookies-video, .cookies-video-blazy-oembed', context).cookiesOverlay('marketing');
        }
        ;
        Drupal.behaviors.cookiesVideo.attach = function(context) {
            var self = this;
            document.addEventListener('cookiesjsrUserConsent', function(event) {
                var service = (typeof event.detail.services === 'object') ? event.detail.services : {};
                if (typeof service.marketing !== 'undefined' && service.marketing)
                    self.consentGiven(context);
                else
                    self.consentDenied(context);
            });
    ...
    if (Drupal.behaviors.cookiesVideoEmbedField) {
        Drupal.behaviors.cookiesVideoEmbedField.consentDenied = function(context) {
            $('iframe.cookies-video-embed-field', context).cookiesOverlay('marketing');
        }
        ;
        Drupal.behaviors.cookiesVideoEmbedField.attach = function(context) {
            var self = this;
            document.addEventListener('cookiesjsrUserConsent', function(event) {
                var service = (typeof event.detail.services === 'object') ? event.detail.services : {};
                if (typeof service.marketing !== 'undefined' && service.marketing)
...
;(function(Drupal) {
    ...
    Drupal.behaviors.cookiesGtag = {
        consentGiven: function() {
            var scriptIds = ['cookies_gtag_gtag', 'cookies_gtag_gtag_ajax', 'cookies_gtag_gtm'];
...
;(function(Drupal, $, Cookies) {
    ...
    Drupal.behaviors.cookiesGtag.consentDenied = function(context) {
        ...
        var cookie_domain = 'college.police.uk';
...
// Script para carregar o vídeo quando o modal abrir
document.addEventListener('DOMContentLoaded', function() {
    const videoModal = document.getElementById('videoModal');
    const videoIframe = document.getElementById('acipolVideo');
 ...
 // Contadores Animados
  ...
  const sections = document.querySelectorAll(".acipol-section");
  ...
  buttons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Anima contadores se for a seção "Nossos Números"
      if (btn.dataset.section === "numeros") animateCounters();
    });
  });
...
// script de hora
            ...
            function updateDateTime() {
                ...
                const dataHora = `${dias[now.getDay()]}, ${now.getDate().toString().padStart(2, '0')} de ${meses[now.getMonth()]} de ${now.getFullYear()} | ${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}:${now.getSeconds().toString().padStart(2, '0')}`;
                document.getElementById('dateTimeDisplay').textContent = dataHora;
            }
            setInterval(updateDateTime, 1000);
            updateDateTime();

            document.getElementById('languageSelect').addEventListener('change', (e) => {
                alert(e.target.value === 'en'
                    ? 'Language changed to English (simulação)'
                    : 'Idioma alterado para Português (simulação)');
            });

// Ativar link ativo automaticamente
document.addEventListener('DOMContentLoaded', function() {
        ...
        title.addEventListener('click', function() {
            ...
            accordions.forEach(otherAccordion => {
                if (otherAccordion !== accordion) {
                    otherAccordion.querySelector('.accrodion-content').style.display = 'none';
                    otherAccordion.classList.remove('active');
                }
            });
            
            // Abre/fecha o accordion clicado
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    categoryItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            // Aqui você pode adicionar a lógica para carregar o conteúdo do curso
            const courseName = this.querySelector('.category-name').textContent;
            console.log('Curso selecionado:', courseName);
        });
    });
    
    // Efeito de entrada stagger para os cards
